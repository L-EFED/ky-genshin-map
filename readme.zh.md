...
一.
index.html 文件是整个 web 应用的入口文件，主要功能包括：
定义 HTML 文档结构：为应用提供基础框架
初始化页面元信息：设置视口、字符编码等
加载必要资源：引入 CSS 样式和 JavaScript 文件
添加页面样式：定义全局和自定义样式
集成分析工具：包含阿里云日志服务(Aplus)的跟踪代码
...
2. 头部元信息 (head 部分)
视口设置：确保页面宽度与设备宽度一致，适配移动端
字符编码：设置为 UTF-8 以支持中文
页面描述：SEO 优化，描述页面内容
页面标题：显示在浏览器标签页上

3. 资源加载
CSS 样式：引入 uno.css 样式表
网站图标：设置普通和苹果设备的 favicon
主 JavaScript：引入 index.js 作为应用主逻辑入口
...
二.
Store.ts 组件分析
这个store.ts文件是一个全局状态管理模块，主要功能包括：

初始化全局状态：
使用valtio的proxy创建了一个全局状态对象store
包含了CanvasKit实例、地图数据、区域信息等核心状态

核心状态结构：
canvaskit: CanvasKit WASM实例
mapData: 地图二进制数据
mapInfo: 当前地图信息
areaItems: 按类型和图标组织的区域物品
activeAreaItems: 当前激活的物品集合
activeTopArea: 当前顶级区域
activeSubArea: 当前子区域

主要功能函数：
init(): 初始化函数，加载CanvasKit和压缩的地图数据
activateArea(): 激活指定区域并更新相关状态
updateMapInfo(): 更新当前地图信息
updateAreaItems(): 更新区域物品数据
activateAreaItem()/inactivateAreaItem(): 激活/取消激活特定区域物品
